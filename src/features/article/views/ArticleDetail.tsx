  ...
  return (
    <>
      <Stack spacing={4}>
        ...
        <Stack direction="row">
          <Stack spacing="1px">
            <DetailItem label="コンテンツID" value={articleId as string} />
            <DetailItem label="タイトル" value={articleDetail?.data.title as string} />
            <DetailItem
              label="登録日時"
              value={formatDateTime(articleDetail?.data.created_at) as string}
            />
            <DetailItem label="カテゴリ名" value={articleDetail?.data.category} />
