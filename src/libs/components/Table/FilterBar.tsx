...
type SelectFilterColumn<F extends FieldValues> = { type: "select" } & Partial<
  SelectProps<F>
>;
...
export type FilterColumn<F extends FieldValues> = FilterColumnBase<F> &
  (
    | InputFilterColumn
    | SelectFilterColumn<F>
    | CustomFilterColumn<F>
    | DateFilterColumn<F>
  );
...
function getColumnByField<F extends FieldValues>(
  columns: FilterColumn<F>[],
  field: keyof F
) {
...
export function FilterBar<FilterInput extends FieldValues, TData>({
  ...
  const columnGroupFields = columns.filter(
    (column) => column.fieldOptions?.groupField
  );
  const columnNotGroupFields = columns.filter(
    (column) => !column.fieldOptions?.groupField
  );
    ...
    if (type === "text") {
      return (
        <Stack spacing={2} direction="row" alignItems="center" key={field}>
          ...
          {hasLine && (
            <Box height="40px" width="1px" bgcolor={base.separate_nav} />
          )}
        </Stack>
      );
    }
