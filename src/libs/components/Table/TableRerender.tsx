...
import {
  TABLE_CELL_HEADER_HEIGHT,
  TABLE_CELL_ROW_HEIGHT,
} from "./ReactTable/TableRender";
...
function TableRenderer<T extends RowData>({
  ...
  return (
    <>
      <MuiTable {...tableProps} sx={{ background: base.white }}>
        {!noHeader && (
          <TableHead sx={{ backgroundColor: base.separate_contents }}>
            {instance[tableSide.headerGroupKey as "getHeaderGroups"]().map(
              (headerGroup) => (
